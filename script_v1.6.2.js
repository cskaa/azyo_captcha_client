class AzyoCaptcha{
  ...
  constructor(id, trigger, holder, onfinish){
    ...
    $.ajax({
      type: "POST",
      enctype: 'JSON',
      url: 'http://platform.azyo.io:5003/secret_code_check',
      //headers: { 'Access-Control-Allow-Origin': 'https://platform.azyo.io:5003/secret_code_check' },
                    ...
                    <div class="card-body">
                      ...
                      <div id="vid" class="justify-content-center d-flex m-0 p-0 vh-75 vw-75" >
                        ...
                        <div style="position: relative;">
                          ...
                          <img id = "eyeblink" src="https://cdn.jsdelivr.net/gh/cskaa/azyo_captcha_client/images/eyeblink.gif" height="100" width="100" style="display: none;position:absolute;top:0;right:0;z-index:1000;outline: 2px solid white;">
                        </div>
                      </div>
                      <br>
  ...
  set_class_vars(){
    ...
    this.choice = this.getRandomInt(1,3)
    // this.choice = 3
    ...
    this.eyeblink_element = document.getElementById("eyeblink")
    ...
    this.EAR_THRESHOLD = 0.27;
    this.blinkCount = 0
    this.longBlinkCounter = 0
    this.totblink = this.getRandomInt(4,6)
  ...
  drawUserInstr(current_task){
    ...
    if (current_task == "fingersOnLips"){
      ...
      this.drawTheDots()
    }else if (current_task == "detectBlinkEvents"){
      this.inst.innerHTML = "Please Blink "+ this.totblink +" times.<br>Times blinked: "+ this.longBlinkCounter
      this.eyeblink_element.style.display = "block"
    }
  }
  ...
  connectTheDots(results, tasknum){

    if (this.drawCanvasElement.style.display !== "block"){
      this.drawCanvasElement.style.display = "block"
    }
    ...
    if(typeof results.rightHandLandmarks === 'undefined'){
      ...
      let x = results.rightHandLandmarks[8].x
      let y = results.rightHandLandmarks[8].y
      ...
      if (this.dotIndex+1 < this.dots.length){
        ...
        if(typeof results.faceLandmarks === 'undefined'){
          ...
          this.pause_text = "Face was obstructed, please try again"
          this.pause_all = 1
  ...
  getEucledianDistance(x1, y1, x2, y2) {
    return Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
  }
  
  getEAR(upper, lower) {
    // console.log(upper[5][0], upper[5].x, upper[5]["x"])
    // console.log(this.getEucledianDistance(upper[5][0], upper[5][1], lower[4][0], lower[4][1]))
    return (
      (this.getEucledianDistance(upper[5]["x"], upper[5]["y"], lower[4]["x"], lower[4]["y"]) + this.getEucledianDistance(upper[3]["x"], upper[3]["y"], lower[2]["x"], lower[2]["y"])) /
      (2 * this.getEucledianDistance(upper[0]["x"], upper[0]["y"], upper[8]["x"], upper[8]["y"]))
    );
  }

  isVoluntaryBlink(blinkDetected) {
  ...
  getAnnotationMapped(face_landmarks){

    // https://github.com/tensorflow/tfjs-models/blob/b5d49c0f5ba2057cc29b40317126c5f182495f96/face-landmarks-detection/src/mediapipe-facemesh/keypoints.ts
    // rightEyeUpper0: [246, 161, 160, 159, 158, 157, 173],
    // rightEyeLower0: [33, 7, 163, 144, 145, 153, 154, 155, 133],
    // leftEyeUpper0: [466, 388, 387, 386, 385, 384, 398],
    // leftEyeLower0: [263, 249, 390, 373, 374, 380, 381, 382, 362],

    let rightEyeUpper0 = [246, 161, 160, 159, 158, 157, 173]
    let rightEyeLower0 = [33, 7, 163, 144, 145, 153, 154, 155, 133]
    let leftEyeUpper0 = [466, 388, 387, 386, 385, 384, 398]
    let leftEyeLower0 = [263, 249, 390, 373, 374, 380, 381, 382, 362]

    let annotate = (indexs, landmarks) => indexs.map(index => landmarks[index])

    // NOTE: Error in docs, rightEyeLower0 is mapped to rightEyeUpper0 and vice-versa
    let lowerRight = annotate(rightEyeUpper0, face_landmarks);
    let upperRight = annotate(rightEyeLower0, face_landmarks);

    const rightEAR = this.getEAR(upperRight, lowerRight);
    
    // TODO: log this prediction
    let lowerLeft = annotate(leftEyeUpper0, face_landmarks);
    let upperLeft = annotate(leftEyeLower0, face_landmarks);
    
    // console.log(upperLeft, lowerLeft)

    const leftEAR = this.getEAR(upperLeft, lowerLeft);

    // console.log(rightEAR, leftEAR)

    return {rightEAR: rightEAR, leftEAR: leftEAR}
  }

  detectBlinkEvents(face_landmarks, tasknum) {
    if(typeof face_landmarks === 'undefined'){
      console.log("face landmarks undefined for blink")
    }
    else{

      let {rightEAR, leftEAR} = this.getAnnotationMapped(face_landmarks)
      // console.log(rightEAR, leftEAR)
      ...
      if (this.lastBlinkFlag == blinked){
        ...
        if (this.longBlinkCounter > this.totblink) {
          this.taskCompleteFlag(tasknum)
          this.longBlinkCounter = 0;
        }
      }
  ...
  increment_succ(id, onfinish){
    $.ajax({
      type: "POST",
      enctype: 'JSON',
      url: "http://platform.azyo.io:5003/success_count",
     // headers: { 'Access-Control-Allow-Origin': 'https://platform.azyo.io:5003/success_count' },
      data: JSON.stringify({'secret': id, 'domain': window.location.hostname}),
      processData: false,
      'contentType': 'application/json',
      cache: false,
  ...
  mainController(results, onfinish, id) {
    if (this.pause_all){
      ...
      this.eyeblink_element.style.display = "none"
    ...
    var array_of_tasks = [
      ...
      [
        ()=> { this.detectBlinkEvents(results.faceLandmarks,0) },
        ()=> { this.connectTheDots(results,1) },
      ]
    ]
